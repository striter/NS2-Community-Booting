...
function GUIDeathStats:Initialize()
	...
	self.titleShadow:SetText("上次存活数据")
...
function GUIDeathStats:SendKeyEvent(key, down)
	...
	if GetIsBinding(key, "RequestMenu") and
			GetAdvancedOption("deathstats") > 0 and
			not EndStatsVisible and
			(player and teamNumber == kTeam1Index or teamNumber == kTeam2Index) and
			not ChatUI_EnteringChatMessage() and
			not MainMenu_GetIsOpened() and
			not PlayerUI_IsOverhead() then
...
function GUIDeathStats:SetStats()
	...
	if statsTable ~= nil then
		self:AddRow("精准度", printNum(statsTable.lastAcc) .. "%")
		if statsTable.lastAccOnos > -1 then
			self:AddRow("剔除Onos命中", printNum(statsTable.lastAccOnos) .. "%")
		end
		self:AddRow("玩家伤害", printNum(statsTable.pdmg))
		self:AddRow("建筑伤害", printNum(statsTable.sdmg))
		if statsTable.kills > 0 then
			self:AddRow("击杀", printNum(statsTable.kills))
		end
		self:AddRow()
		self:AddRow("精准度(当前)", printNum(statsTable.currentAcc) .. "%")
		if statsTable.currentAccOnos > -1 then
			self:AddRow("剔除Onos命中", printNum(statsTable.currentAccOnos) .. "%")
		end
	end
