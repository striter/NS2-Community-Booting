...
Script.Load("lua/CNSpectator/TinymanSpectatorMode.lua")
---------------
...
local function UpdateSpectatorMode(self, input)
    ...
    if self.timeFromLastAction > kDeltatimeBetweenAction then
        ...
        if not self.thirdPersonPressed and bit.band(input.commands, Move.Reload) ~= 0 then
            self.thirdPersonPressed = true
            self:SetIsThirdPerson(self:GetIsThirdPerson() and 0 or 0.3)
        else
            self.thirdPersonPressed = false
        end
    end
...
--------------
    ...
    self.controller:SetCollisionEnabled(false)
--------------
...
function Spectator:OnClientUpdated(client, isPickup)
...
function Spectator:OnProcessMove(input)
    ...
    if self.specMode == kSpectatorMode.Tinyman then
        ...
        FreeLookMoveMixin.UpdateMove(self,input)
        OverheadMoveMixin.UpdateMove(self,input)
        FollowMoveMixin.UpdateMove(self,input)
        MinimapMoveMixin.UpdateMove(self,input)
    end
...
function Spectator:GetTargetsToFollow(includeCommandStructure)
    ...
    if table.icount(targets) < 1 and not includeCommandStructure then
...
function Spectator:GetHasController()
    return true
end
