...
do
    ...
    function Shine.GetClientByName(Name)
        if not IsType(Name, "string") or #Name == 0 then
            return nil
        end
...
do
    ...
    local function SafeToNumber(String)
        if IsType(String, "number") then
            return String
        end
        if not StringMatch(String, "^[0-9]+$") then
            return nil
        end
...
function Shine:GetClientsByGroup(Group)
    if Group ~= "guest" and not self.UserData.Groups[Group] then
        return {}
    end
