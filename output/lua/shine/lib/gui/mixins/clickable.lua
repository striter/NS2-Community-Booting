...
function Clickable:OnMouseDown(Key, DoubleClick)
    ...
    if not self:GetIsVisible() or not self:MouseInControl() then
        return
    end
    if not GetClickMethod(self, Key) then
        return
    end
...
function Clickable:OnMouseUp(Key)
    if not self:MouseInControl() then
        return
    end

    local Time = Clock()
    if (self.ClickDelay or 0.1) > 0 and (self.NextClick or 0) > Time then
        return true
    end
