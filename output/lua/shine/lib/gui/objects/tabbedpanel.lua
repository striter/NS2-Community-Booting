...
do
    ...
    function TabPanel:SetExpanded(Expanded)
        ...
        if not Expanded and self.Horizontal then
            return
        end
...
do
    local LayoutSetup = {
        ...
        [true] = function(self, InternalLayout)
            ...
            -- Invalidate immediately to avoid the button being positioned incorrectly for a frame.
                    :ToListener(function()
                TabsLayout:InvalidateLayout(true)
            end)
                    :BindProperty()
        end,
    ...
    function TabPanel:SetHorizontal(Horizontal)
        ...
        if self.Horizontal == Horizontal then
            return
        end
...
function TabPanel:ForceTabRefresh(Index)
    if self.ActiveTab ~= Index or not self.Tabs[Index] then
        return
    end
