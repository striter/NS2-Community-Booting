...
function Panel:SetTitle(Title)
    if not SGUI.IsValid(self.TitleLabel) then
        return
    end
...
local function UpdateMaxSize(Child)
    ...
    if not Parent.ScrollParent or not Child:GetIsVisible() then
        return
    end
...
function Panel:ScrollIntoView(Child, ForceInstantScroll)
    ...
    if not self.ScrollParent or not self.ScrollParentPos then
        return
    end
...
function Panel:OnAddScrollbar()
    if not self.Layout or not self.ResizeLayoutForScrollbar then
        return
    end
...
function Panel:SetMaxWidth(MaxWidth)
    ...
    if not self.ShowScrollbar or not self.HorizontalScrollingEnabled then
        return
    end
...
function Panel:DragClick(Key, DoubleClick)
    ...
    local Size = self:GetSize()
    if not self:MouseInBounds(self.Background, Size.x, Size.y * 0.05) then
        return
    end
...
function Panel:OnMouseDown(Key, DoubleClick)
    ...
    if self.CroppingBox and not self:HasMouseEntered() then
        return
    end
    ...
    if self:DragClick(Key, DoubleClick) then
        return true, self
    end
