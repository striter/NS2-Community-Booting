...
function TextEntry:AllowChar(Char)
    if not Char:IsValidUTF8() then
        return false
    end
    if self:ShouldAllowChar(Char) == false then
        return false
    end
...
function TextEntry:AddCharacter(Char, SkipUndo)
    if not self:AllowChar(Char) then
        return false
    end
...
function TextEntry:RemoveWord(Forward)
    ...
    if Forward then
        if self.Column == StringUTF8Length(self.Text) then
            return
        end
...
function TextEntry:RemoveCharacter(Forward)
    ...
    if self.Column == 0 and not Forward then
        return
    end
...
function TextEntry:OnMouseLeave()
    ...
    if self.Enabled or not self.Highlighted then
        return
    end
...
do
    ...
    function TextEntry:OnTab()
        if not self.AutoCompleteHandler then
            return
        end
        ...
        if not Success or not NewState then
            return
        end
...
function TextEntry:PlayerKeyPress(Key, Down)
    ...
    if SGUI:IsControlDown() then
        ...
        if Down and Key == InputKey.V then
            ...
            for i = 1, #Chars do
                if not self:AddCharacter(Chars[i], true) then
                    break
                end
            end
