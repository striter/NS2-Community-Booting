...
function ControlMeta:SetTopLevelWindow(Window)
    if Window == self.TopLevelWindow then
        return
    end
...
function ControlMeta:SetStencilled(Stencilled)
    if Stencilled == self.Stencilled then
        return
    end
...
do
    ...
    function ControlMeta:SetIsVisible(IsVisible)
        ...
        if self.Background.GetIsVisible and self.Background:GetIsVisible() == IsVisible then
            return
        end
        ...
        if not SGUI:IsWindow(self) then
            return
        end
        ...
        if IsVisible then
            ...
            if SGUI.FocusedWindow ~= self then
                return
            end
...
function ControlMeta:UpdateAbsolutePositionChildren()
    if not self.ChildrenByPositionType then
        return
    end
...
function ControlMeta:SetPositionType(PositionType)
    ...
    if OldPositionType == PositionType then
        return
    end
...
function ControlMeta:PreComputeHeight(Width)
    if not self.AspectRatio or not self.AutoSize then
        return
    end
...
do
    ...
    function ControlMeta:HandleEasing(Time, DeltaTime)
        if not self.EasingProcesses or self.EasingProcesses:IsEmpty() then
            return
        end
...
local function Easer(Table, Name)
    return setmetatable(Table, { __tostring = function()
        return Name
    end })
end
...
function ControlMeta:GetEasing(Type, Element)
    if not self.EasingProcesses then
        return
    end
...
do
    ...
    local function NoOpHighlighting()
    end
...
function ControlMeta:HandleLayout(DeltaTime)
    ...
    for i = 1, 5 do
        if not self.LayoutIsInvalid then
            break
        end
...
do
    ...
    function ControlMeta:HideTooltip()
        if not SGUI.IsValid(self.Tooltip) then
            return
        end
...
function ControlMeta:SetHighlighted(Highlighted, SkipAnim)
    if not not Highlighted == not not self.Highlighted then
        return
    end
...
function ControlMeta:HandleMouseState()
    if not self.MouseStateIsInvalid or not SGUI.IsMouseVisible() then
        return
    end
...
function ControlMeta:RequestFocus()
    if not self.UsesKeyboardFocus then
        return
    end
