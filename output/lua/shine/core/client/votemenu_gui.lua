...
function VoteMenu:SetPage(Name, IgnoreAnim)
    local Page = self.Pages[Name]
    if not Page or not Page.Populate then
        return
    end
...
local function AddButton(self, Pos, Anchor, Text, DoClick)
    ...
    Button.OnAnimationComplete = function()
        Button:SetHighlightOnMouseOver(true, true)
    end
    Button.ClickDelay = 0
...
VoteMenu:AddPage("Main", function(self)
    ...
    if not self.CanViewAdminMenu then
        return
    end
