...
function ConfigMenu:Create()
    ...
    self.Menu.OnPreTabChange = function(Window)
        ...
        if not Tab or not Tab.OnCleanup then
            return
        end
...
function ConfigMenu:Close()
    if not self.Menu or not self.Visible then
        return
    end
...
function ConfigMenu:OpenOnSettingsTab(TabName)
    ...
    if not SGUI.IsValid(self.SettingsTabs) then
        return
    end
