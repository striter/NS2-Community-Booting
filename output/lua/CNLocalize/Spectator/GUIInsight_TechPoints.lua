...
function GUIInsight_TechPoints:Initialize()
    ...
    self.techPointBackground:SetTexturePixelCoordinates(GUIUnpackCoords({0,0,512,512}))
    ...
    topBar:SetTexturePixelCoordinates(GUIUnpackCoords({0,2*hBarCoords.y,hBarTextureWidth,3*hBarCoords.y}))
    ...
    leftBar:SetTexturePixelCoordinates(GUIUnpackCoords(leftBarCoords))
    ...
    corner:SetTexturePixelCoordinates(GUIUnpackCoords(cornerCoords))
    ...
    top:SetTexturePixelCoordinates(GUIUnpackCoords(topCoords))
    ...
    bottom:SetTexturePixelCoordinates(GUIUnpackCoords(bottomCoords))
    ...
    if isVisible then
        ...
        if numTechPoints > 0 then
            ...
            if table.icount(self.techPointList) ~= numTechPoints then
                ...
                --self.techPointBackground:SetTexturePixelCoordinates(GUIUnpackCoords({0,0,225 * 2,40*numTechPoints * 2}))
                ...
                --self.leftBar:SetTexturePixelCoordinates(GUIUnpackCoords({2*vBarCoords.x,0,3*vBarCoords.x,vBarTextureHeight}))
           end
            
            local currentPointIndex = 1
            for index, techPoint in ipairs(self.techPointList) do
...
function GUIInsight_TechPoints:UpdateTechPoint(techPoint, techPointRecord, currentPointIndex)
    ...
    if teamChanged then
        ...
        icon:SetTexturePixelCoordinates(GUIUnpackCoords(GetTextureCoords(team, techId)))
        ...
        if team > 0 then        
            ...
            if not built then
                ...
                local text = string.format("%s %s", location,Locale.ResolveString("TECHPOINT_CLAIMED"))
            ...
            if storedValues.Team > 0 then
                local text = string.format("%s %s",location,Locale.ResolveString("TECHPOINT_DESTROYED"))
...
function GUIInsight_TechPoints:CreateBackground()
    ...
    background:SetTexturePixelCoordinates(GUIUnpackCoords({0,0,432,80}))
    ...
    typeIcon:SetTexturePixelCoordinates(GUIUnpackCoords({0,160,80,240}))
...
function GUIInsight_TechPoints:ResizeList(list, listCount, GUIItem)
    ...
    while table.icount(list) > listCount do
